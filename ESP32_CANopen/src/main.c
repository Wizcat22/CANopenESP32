...
const esp_timer_create_args_t coMainTaskArgs = {
    .callback = &coMainTask, /*Timer Interrupt Callback */
    .name = "coMainTask"}; /* Optional Task Name for debugging */
//Timer Handle
...
void mainTask(void *pvParameter)
{
  ...
  while (reset != CO_RESET_APP)
  {
    ...
    while (reset == CO_RESET_NOT)
    {
      ...
      ESP_LOGI("TEST","Angle: %f", getAngle());
      ...
      {
        enableDriftCompensation(CO);
        DCenabled = 1;
      }
      if (DCenabled == 1){
        if (isDriftCompenstaionComplete()){
          disableDriftCompensation(CO);
          DCenabled = 0;
        }
      }
      if (gpio_get_level(GPIO_INPUT_IO_1)){
        enableAngleToZero(CO);
      }
      if (confirmAngleSetToZero()){
        disableAngleToZero(CO);
      }
